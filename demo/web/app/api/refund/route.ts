...
export async function POST() {
  try {
    ...
    const { RPC_URL, OPERATOR_PRIVATE_KEY, RELAYER_PRIVATE_KEY, AUTH_CAPTURE_ESCROW, OPERATOR_REFUND_COLLECTOR, DEMO_TOKEN, MERCHANT, PAYER } = env;
    ...
    if (!AUTH_CAPTURE_ESCROW || !OPERATOR_REFUND_COLLECTOR || !DEMO_TOKEN || !MERCHANT || !PAYER) throw new Error('Missing required envs');
    ...
    const refundCollector = getAddress(OPERATOR_REFUND_COLLECTOR);
    const payer = getAddress(PAYER);
    const merchant = getAddress(MERCHANT);
    const amount = 5n * 10n ** 15n; // 0.005
    ...
    const refundHash = await operatorWallet.writeContract({ address: escrow, abi: ESCROW_ABI, functionName: 'refund', args: [paymentInfo, amount, refundCollector, '0x'], nonce: opNonce });
    ...
    return NextResponse.json({ addresses: { escrow, token, operator: operatorAccount.address, payer, merchant, tokenStore }, txs: { refundHash }, balancesBefore, balancesAfter });
  } catch (e: any) {
